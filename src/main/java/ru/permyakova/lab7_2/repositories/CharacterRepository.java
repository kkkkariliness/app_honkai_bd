 ...
 * Предоставляет методы для выполнения операций CRUD и пользовательские запросы к базе данных для персонажей.
 */
public interface CharacterRepository extends JpaRepository<Character, UUID> {
     ...
     * @param moneyDonat Минимальная сумма доната для поиска.
     * @return {@link Optional}, содержащий случайного персонажа, если найден, иначе пустой {@link Optional}.
     */
    @Query(value = "SELECT * FROM character c WHERE c.ch_money_donat > :moneyDonat ORDER BY RANDOM() LIMIT 1", nativeQuery = true)
